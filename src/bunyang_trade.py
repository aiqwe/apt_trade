...
def _sub_task(lawd_cd, deal_ymd):
    ...
    # pageNo
    # numOfRows
    sentinel = get_api_data(base_url=URLDictionary.URL['bunyang_trade'], LAWD_CD=lawd_cd, DEAL_YMD=deal_ymd)
    ...
    if total_cnt > 0:
        for i in range(1, iteration + 1):
            response = get_api_data(base_url=URLDictionary.URL['bunyang_trade'], LAWD_CD=lawd_cd, DEAL_YMD=deal_ymd, pageNo=i, numOfRows=1000)
            if i == 1:
...
if __name__ == '__main__':
    ...
    batch_manager(task_id=get_task_id(__file__, this_month), key=date_id, func=main_task, month=last_month, date_id=date_id)
    batch_manager(task_id=get_task_id(__file__, last_month), key=date_id, func=main_task, month=this_month, date_id=date_id)